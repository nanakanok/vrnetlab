...
class Ubuntu_vm(vrnetlab.VM):
    ...
    def create_boot_image(self):
        ...
        with open("/bootstrap_config.yaml", "w") as cfg_file:
            ...
            cfg_file.write("runcmd:\n")
            cfg_file.write("  - apt-get update\n")
       	    cfg_file.write("  - apt-get install -y linux-modules-extra-$(uname -r) rdma-core infiniband-diags ibverbs-providers libibverbs-dev rdmacm-utils qperf\n")
       	    cfg_file.write("  - apt-get clean\n")
            cfg_file.write("  - touch /etc/cloud/cloud-init.disabled\n")
